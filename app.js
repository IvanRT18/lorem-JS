...
//Seleccion de los elementos que vamos a utilizar
const form = document.querySelector(".lorem-form");
const amount = document.getElementById("amount");
const article = document.querySelector(".lorem-text");

//Cuando se haga click en generar, osea en submit el form
form.addEventListener("submit", function (e) {
  e.preventDefault(); //Prevenimos que se recargue la pagina al generar un parrafo
  const value = parseInt(amount.value); //Convertimos el string del input en numero entero (int)

  const randomNumber = Math.floor(Math.random() * text.length); //Numero random

  //Se generará un solo parrafo si, amount es iguala a NaN, menor a 0, o mayor a 9
  if (isNaN(value) || value < 0 || value > 9) {
    article.innerHTML = `<p class="result">${text[randomNumber]}</p>`;
  } else {
    //Si es un numero entre 1 y 9, se mostrara ese numero de parrafos
    article.innerHTML = "";
    const tempArray = text.slice(0, value); //Usando slice y forEach
    tempArray.forEach(function (texto, index) {
      article.innerHTML += `<p class="result">${text[index]}</p>`;
    });
  }
});
